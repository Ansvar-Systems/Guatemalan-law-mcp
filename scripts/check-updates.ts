#!/usr/bin/env tsx
/**
 * Guatemalan Law MCP -- Data Freshness Checker
 *
...
const PORTAL_URL = 'https://guatemala.justia.com';
const PORTAL_NAME = 'Guatemala Law (Justia)';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/guatemalan-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Guatemalan Law MCP -- Data Freshness Check');
