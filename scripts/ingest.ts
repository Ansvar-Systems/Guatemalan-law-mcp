#!/usr/bin/env tsx
/**
 * Guatemalan Law MCP -- Census-Driven Ingestion Pipeline
 *
 * Reads data/census.json and fetches + parses every ingestable law
 * from guatemala.justia.com (HTML full-text pages).
 *
 * Pipeline per law:
 *   1. Fetch HTML page from guatemala.justia.com
 ...
 *
 * Data source: guatemala.justia.com
...
import { parseGTLawHtml, type ActIndexEntry, type ParsedAct } from './lib/parser.js';
...
async function main(): Promise<void> {
  ...
  console.log('Guatemalan Law MCP -- Ingestion Pipeline (Census-Driven)');
  console.log('========================================================\n');
  console.log('  Source: guatemala.justia.com');
  ...
  for (const law of acts) {
    ...
    try {
      ...
      // Parse HTML
      const parsed = parseGTLawHtml(htmlContent, act);
  ...
  console.log(`\n  Source:      guatemala.justia.com (HTML)`);
