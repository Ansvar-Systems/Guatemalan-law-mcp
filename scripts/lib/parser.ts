/**
 * Guatemalan Law HTML/PDF/Text Parser
 *
 * Parses law text from Justia Guatemala (guatemala.justia.com) or from
 * PDFs. Applies regex-based article parsing for Guatemalan civil law.
 *
 * Guatemala follows Spanish civil law (Roman-Germanic tradition).
 * Laws are published in the Diario de Centro America (official gazette).
 *
...
// SECURITY: execFileSync prevents command injection -- arguments passed as array
...
const ARTICLE_PATTERNS = [
  /(?:^|\n)\s*(?:Art[\u00ed]culo|ART[\u00cdI]CULO|Art\.?)\s+((?:\d+[\s.]*(?:bis|ter|quater)?|\d+[A-Z]?(?:\.\d+)?|[\u00daU]NICO|PRIMERO))\s*[.\u00b0\u00ba]*[-.:;\u2013]?\s*([^\n]*)/gimu,
...
export function parseGTLawPdf(pdfPath: string, act: ActIndexEntry): ParsedAct {
...
export function parseGTLawHtml(html: string, act: ActIndexEntry): ParsedAct {
  return parseGTLawText(html, act);
}
