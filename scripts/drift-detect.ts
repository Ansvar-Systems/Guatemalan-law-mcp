#!/usr/bin/env tsx
/**
 * Drift detection for Guatemalan Law MCP.
 *
 * Checks if upstream Guatemalan Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Guatemalan Law MCP -- Drift Detection');
  console.log('======================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'guatemalan-law-mcp/1.0 drift-detect' },
      });
