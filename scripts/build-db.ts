#!/usr/bin/env tsx
/**
 * Database builder for Guatemala Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Guatemala Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Guatemalan statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'GT');
    insertMeta.run('source', 'guatemala.justia.com');
    insertMeta.run('licence', 'Justia (public access)');
  });
  writeMeta();
